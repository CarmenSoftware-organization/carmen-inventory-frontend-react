...
- **Motion is barely specified.** `globals.css` defines fade/float/pulse keyframes and interaction transitions; there is no documented motion scale, and no `prefers-reduced-motion` story beyond three scattered usages.
- **Focus, keyboard order and screen-reader behaviour are undocumented.** Components carry `focus-visible` rings from shadcn defaults, but no audit has been done.
