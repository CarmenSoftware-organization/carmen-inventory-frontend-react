...
**ขอบเขตจริงเล็กกว่าที่ประเมินไว้ตอนเขียนแผน — อ่านตรงนี้ก่อนเริ่ม**

ตอนเขียนแผนคิดว่าต้องแก้ทั้งการอ่านและการเขียน ~2,358 จุด **ไม่จริง**

- **ฝั่งอ่าน = บังคับ** API ไม่ส่ง flat แล้ว ถ้าไม่แก้หน้าจอจะว่าง
- **ฝั่งเขียน = ไม่บังคับ** `@ExpandRefs` ที่ backend **รับทั้งรูป flat เดิมและ object ใหม่**
  (พิสูจน์ซ้ำหลายรอบใน Task 13/14 ด้วย payload จริงทั้งสองแบบ) → **payload builder
  ของ FE ไม่ต้องแตะเลยก็ได้ ของเดิมยังทำงานถูกต้อง**

**ทำฝั่งอ่านอย่างเดียวก่อน** เหตุผลไม่ใช่แค่ลดงาน แต่เพื่อให้แยกสาเหตุได้:
ถ้าแก้ทั้งสองทางพร้อมกันแล้วมีอะไรพัง จะบอกไม่ได้ว่าพังเพราะการอ่านหรือการเขียน

การแก้ payload ให้ส่งรูป object เป็นงานเสริมที่ทำทีหลังได้ หรือไม่ทำเลยก็ได้
