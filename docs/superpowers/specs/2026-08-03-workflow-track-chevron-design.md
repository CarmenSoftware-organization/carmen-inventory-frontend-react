...
/* .dark — ink เป็นค่าคงที่ ไม่มีตัวแปรให้พลิกตามธีมเหมือน -soft จึงต้องมีคู่
   ...
   pending-soft ต้องมีคู่ตามมา เพราะการลด approved เหลือ 7% ดึงพื้น done ลงมาชน
   พื้น pending พอดี (1.001:1) — ดูหัวข้อ "ข้อจำกัดที่เหลือ" ว่าทำไมเหลือค่าเดียว */
--status-approved-soft:   color-mix(in oklab, var(--status-approved) 7%, var(--card));
--status-pending-soft:    oklch(0 0 0);
...
- **ข้อความ** เป็นสีกลางบนพื้น tint จาง วัดแล้วผ่าน 4.5:1 (WCAG AA — 11px นับเป็น
  ...
  — เป็นตัวกำหนดว่า `--status-approved-soft` ในธีมมืดผสมได้ไม่เกิน 8%
  (`--status-pending-soft` เคยติดเพดานเดียวกันที่ 2.5% แต่ตอนนี้โหมดมืดเป็นดำสนิท
  จึงหลุดจากข้อจำกัดนี้ไปแล้ว — ข้อความในช่อง pending ขยับเป็น 6.24)
...
## ข้อจำกัด / ข้อกังวลที่เหลือ

### palette โหมดมืดคับเกินกว่าจะวาง tint ได้อย่างสบาย

ต้นตอของทุกอย่างในหัวข้อนี้คือ `--muted-foreground` บน `--card` ในธีมมืดเริ่มต้นที่
**4.89:1** เหลือระยะถึงเส้น AA 4.5 แค่ **0.39** ก่อนจะผสม tint อะไรเลย ทุก % ที่ผสม
เพิ่มกินระยะนั้นไปราว 0.09 ดังนั้น "พื้น tint จาง + ข้อความ `--muted-foreground`"
เป็น pattern ที่ธีมมืดของแอปนี้แทบไม่มีที่ให้เล่น ไม่ใช่ข้อจำกัดของ component

ผลที่ตามมาเป็นทอด ๆ: ต้องลด `--status-approved-soft` จาก 14% เหลือ 7% เพื่อกู้ AA
ของช่อง `done` (4.25 → 4.58) แต่การลดนั้นดึงพื้น `done` ลงมาชนพื้น `pending` พอดี
วัด contrast กันเองได้ **1.001:1** — หลัก "ผ่านแล้วเขียว · ยังไม่ถึงเทา" หายไปในโหมดมืด
เหลือแต่ไอคอน `✓` / `○` ที่ยังแยกให้

### ทำไม `--status-pending-soft` โหมดมืดจึงเป็นดำสนิท

วัดครบทั้งแกน luminance แล้วเหลือค่าเดียวจริง ๆ (relative luminance `Y`):

| จุดอ้างอิง (dark) | สี | `Y` |
|---|---|---|
| `--background` (พื้นจริงที่แถบวางอยู่) | `#161616` | 0.00800 |
| `--status-approved-soft` 7% (`done`) | `#202622` | 0.01822 |
| เพดานที่ `--muted-foreground` ยังผ่าน AA 4.5 | — | 0.01937 |

สามเงื่อนไข (แยกจาก `done` ≥1.15 · ข้อความ ≥4.5 · รูปลูกศรเทียบ `--background` ≥1.15)
ตัดกันจนเหลือทางเดียว:

- **สว่างกว่า `done`** ต้องได้ `Y ≥ 0.02845` แต่เพดาน AA อยู่ที่ `0.01937` — เพดาน
  อยู่ต่ำกว่าพื้นที่ที่ต้องไปถึง เงื่อนไข 1 กับ 2 ขัดกันเองตรง ๆ
- **แทรกระหว่าง `--background` กับ `done`** ไม่ได้ ช่องว่างนั้นกว้าง **1.176:1**
  แต่การอยู่ห่างทั้งสองฝั่งฝั่งละ 1.15 ต้องใช้ `1.15 × 1.15 = 1.3225`
- **เข้มกว่า `--background`** จึงเป็นทางที่เหลือ และเพดานฝั่งนี้คือ
  `(0.008 + 0.05) / 0.05 = 1.160:1` ที่ดำสนิทพอดี

สแกน gray ไร้ hue ทั้ง 256 ระดับ ผ่านครบสามข้อแค่สองค่า — `#000000` (1.364 / 6.243 /
1.160) กับ `#010101` (1.356 / 6.205 / 1.153) เลือกตัวแรกเพราะเหลือระยะเผื่อมากกว่า

### ความเปราะที่รับไว้อย่างรู้ตัว

- **ระยะเผื่อของ "รูปลูกศรเทียบ `--background`" เหลือ 0.010** และอยู่ที่ขอบล่างของ
  sRGB แล้ว ถ้า `--background` โหมดมืดถูกทำให้เข้มลงกว่า `oklch(0.20 0 0)` เมื่อไร
  ข้อนี้ตกทันทีโดยไม่มีสีเข้มกว่านี้ให้ถอยไปอีก
- **ทางแก้เชิงโครงสร้างเมื่อถึงตอนนั้น** (ทั้งหมดกินขอบเขตกว้างกว่า component นี้):
  ดัน `--card` / `--background` โหมดมืดให้ห่างกันมากขึ้น · ยก `--muted-foreground`
  โหมดมืดให้สว่างขึ้นเพื่อคืนระยะ AA แล้ว tint จะผสมได้เข้มขึ้น · หรือเลิกผูก
  "รูปลูกศรมองเห็น" ไว้กับ contrast ของพื้น แล้วใช้เส้นขอบ 1px แทน
- **โหมดสว่างของช่อง `pending` แยกจาก `done` ด้วย hue ไม่ใช่ luminance** — วัด
  contrast กันเองได้แค่ 1.031 แต่ `done` เป็นเขียวอ่อน `#def1e4` ส่วน `pending`
  เป็นเทากลาง `#e9e9e9` คนละ hue จึงยังแยกออกด้วยตา (WCAG วัดเฉพาะ luminance
  จึงประเมิน tint สีต่ำกว่าความจริง เหมือนกรณี `current` ในหัวข้อ Accessibility)
...
| `styles/badge-status.css` | เพิ่ม 7 custom property ท้ายบล็อก `:root` (soft 4 + ink 3) และ 5 ตัวท้ายบล็อก `.dark` (ink 3 + approved-soft ที่ต้องลด % + pending-soft ที่ต้องแยกจาก approved) |
