...
import type { SrItemHistoryEntry } from "@/types/store-requisition";
import { formatDate } from "@/lib/date-utils";
import { useProfile } from "@/hooks/use-profile";
import { cn } from "@/lib/utils";

interface SrItemHistorySheetProps {
  readonly history: SrItemHistoryEntry[];
  readonly productName?: string;
}

/**
 * ปุ่มไอคอน git-branch ในคอลัมน์ action ของตารางรายการใบเบิก กดแล้วเปิด sheet
 * แสดง timeline ประวัติ workflow ระดับรายการ (per-item) ในรูปแบบซิกแซกสลับซ้าย/ขวา
 * เรียงล่าสุดขึ้นบนสุด — โครงเดียวกับ PrItemHistorySheet ของใบขอซื้อ
 ...
 * @example
 * <SrItemHistorySheet history={item.history} productName={item.product_name} />
 */
export function SrItemHistorySheet({
  history,
  productName,
}: SrItemHistorySheetProps) {
          ...
          <div className="px-4 pb-4">
            ...
            >
              {reversedHistory.map((entry, i) => {
                const config = SR_ITEM_STATUS_CONFIG[entry.status] ?? {
                  className: "",
                  label: entry.status,
                };
                const isEven = i % 2 === 0;
