...
export default function JournalVoucherForm() {
  ...
  return <div className="space-y-4 pb-10">
    ...
    <header className="bg-card flex flex-wrap items-center justify-between gap-2 rounded-lg border p-2"><div className="flex flex-wrap items-center gap-1"><Button variant="ghost" size="sm" onClick={() => navigate("/accounting/journal-voucher")}><ArrowLeft className="size-4" /> Back</Button><span className="bg-border mx-1 h-5 w-px" aria-hidden="true" /><DocumentListHeader title={loaded?.display_no ?? "New Journal Voucher"} description="General Ledger journal entry" /></div><div className="flex flex-wrap items-center gap-1">{loaded?.jv_status === "posted" && <Button variant="outline" size="sm" onClick={() => runAction("reverse")}><RotateCcw className="size-4" /> Reverse</Button>}{loaded?.jv_status === "post_failed" && <Button variant="outline" size="sm" onClick={() => runAction("retry-post")}><RotateCcw className="size-4" /> Retry post</Button>}{loaded && ["draft", "submitted", "scheduled", "post_failed"].includes(loaded.jv_status) && <Button variant="ghost" size="sm" onClick={() => runAction("void")}><Ban className="size-4" /> Void</Button>}<Button variant="outline" size="sm" disabled={!editable || create.isPending || update.isPending} onClick={save}><Save className="size-4" /> Save draft</Button><Button size="sm" disabled={!editable || submit.isPending} onClick={submitDraft}><Send className="size-4" /> Submit</Button></div></header>
