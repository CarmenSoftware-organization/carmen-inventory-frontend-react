...
export default function ActivityLogComponent() {
  ...
  const [entityTypeFilter, setEntityTypeFilter] = useURL("entity_type");
  ...
  const queryParams = (() => {
    ...
    if (entityTypeFilter) p.entity_type = entityTypeFilter;
  ...
  if (entityTypeFilter) {
    for (const v of entityTypeFilter.split(",")) {
      const match = ENTITY_TYPE_OPTIONS.find((o) => o.value === v);
      if (match) {
        activeFilters.push({
          key: `entity_type:${v}`,
          label: match.label,
          onRemove: () => {
            const next = entityTypeFilter
              .split(",")
              .filter((val) => val !== v)
              .join(",");
