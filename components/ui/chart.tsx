...
/**
 * ดึง config ของรายการจาก payload ของ recharts ตาม key ที่กำหนด
 *
 * รองรับทั้ง payload ตรง ๆ และ nested payload.payload (กรณี Pie/Bar)
 * ถ้าค่าของ key เป็น string จะใช้เป็น configLabelKey มิฉะนั้น fallback
 * ไปที่ key เดิม
 *
 * @param config - ChartConfig ของ chart
 * @param payload - payload object จาก recharts (unknown shape)
 * @param key - key เริ่มต้นสำหรับ lookup
 * @returns config entry หรือ undefined
 * @example
 * ```ts
 * const itemConfig = getPayloadConfigFromPayload(config, item, "revenue");
 * ```
 */
