 ...
 *
 * `pending` ใช้ --status-pending-soft ซึ่งเป็นเทาไร้ hue ไม่ใช่ tint ของ
 * --status-draft — ในเชิงความหมาย "ยังไม่ถึง" คือยังไม่มีสถานะ ไม่ใช่สถานะสีเทา ·
 * เดิมใช้ bg-muted เปล่า ๆ แต่ในโหมดมืดมันต่างจากพื้นหน้าแค่ 1.13:1 รูปลูกศรจึง
 * แทบไม่ปรากฏ (ช่อง pending มักเป็นช่องสุดท้าย ไม่มีเพื่อนบ้านช่วยตัดขอบ)
 * token นี้คนละค่ากันสองธีม: สว่างเป็นเทาอ่อน (--foreground 2% เข้า --muted)
 * มืดเป็นดำสนิท เพราะพื้นช่อง done ในโหมดมืดถูกลดเหลือ tint 7% จนมาชนเทาช่องนี้
 * พอดี (1.001:1) — ที่มาของตัวเลขและเหตุผลว่าทำไมเหลือค่าเดียว อยู่ใน
 * styles/badge-status.css
 *
